...
(library (protobuf compile codegen)
  ...
  ;; unwrapping identifiers generated by generate-temporaries 
  ;; may return non portale representative symbol. so make it
  ;; manually.
  (define gensym-values
    (let ((counter -1))
      (lambda vars
	(apply values
	       (map (lambda (v)
  ...
  (define (protoc:generate-message message naming-context)
	...
	(define (,(extension-accessor-name message) ,e0 ,e1)
	  (protobuf:message-extension 
	   ,e0 
	   (record-type-descriptor ,(message-type-name message)) ,e1))
